...
/* `#END` */
    ...
    *******************************************************************************/
    CY_ISR(Pump_RXISR)
    {
    ...
    #ifdef Pump_RXISR_ENTRY_CALLBACK
        Pump_RXISR_EntryCallback();
    #endif /* Pump_RXISR_ENTRY_CALLBACK */
        ...
        do
        {
            ...
            readStatus = Pump_RXSTATUS_REG;
            /* Copy the same status to readData variable for backward compatibility support 
            *  of the user code in Pump_RXISR_ERROR` section. 
            */
            readData = readStatus;

            if((readStatus & (Pump_RX_STS_BREAK | 
                            Pump_RX_STS_PAR_ERROR |
                            Pump_RX_STS_STOP_ERROR | 
                            Pump_RX_STS_OVERRUN)) != 0u)
            {
                /* ERROR handling. */
                Pump_errorStatus |= readStatus & ( Pump_RX_STS_BREAK | 
                                                            Pump_RX_STS_PAR_ERROR | 
                                                            Pump_RX_STS_STOP_ERROR | 
                                                            Pump_RX_STS_OVERRUN);
            ...
            #ifdef Pump_RXISR_ERROR_CALLBACK
                Pump_RXISR_ERROR_Callback();
            #endif /* Pump_RXISR_ERROR_CALLBACK */
            }
            
            if((readStatus & Pump_RX_STS_FIFO_NOTEMPTY) != 0u)
            {
                ...
                {
                    if((readStatus & Pump_RX_STS_MRKSPC) != 0u)
                    {
                        if ((readStatus & Pump_RX_STS_ADDR_MATCH) != 0u)
                        {
                ...
                if(increment_pointer != 0u)
                {
                    ...
                    if(Pump_rxBufferWrite >= Pump_RX_BUFFER_SIZE)
                    {
                    ...
                    if(Pump_rxBufferWrite == Pump_rxBufferRead)
                    {
                        ...
                        #endif /* (Pump_FLOW_CONTROL != 0u) */
                    }
                }
            }
        }while((readStatus & Pump_RX_STS_FIFO_NOTEMPTY) != 0u);
    ...
    #ifdef Pump_RXISR_EXIT_CALLBACK
        Pump_RXISR_ExitCallback();
    #endif /* Pump_RXISR_EXIT_CALLBACK */
    ...
    *******************************************************************************/
    CY_ISR(Pump_TXISR)
    {
    ...
    #ifdef Pump_TXISR_ENTRY_CALLBACK
        Pump_TXISR_EntryCallback();
    #endif /* Pump_TXISR_ENTRY_CALLBACK */
             ...
             ((Pump_TXSTATUS_REG & Pump_TX_STS_FIFO_FULL) == 0u))
        {
            ...
            if(Pump_txBufferRead >= Pump_TX_BUFFER_SIZE)
            {
    ...
    #ifdef Pump_TXISR_EXIT_CALLBACK
        Pump_TXISR_ExitCallback();
    #endif /* Pump_TXISR_EXIT_CALLBACK */
