...
/*******************************************************************************
* Function Name: Clock_Pump_Start
********************************************************************************
*
* Summary:
...
void Clock_Pump_Start(void) 
{
    ...
    Clock_Pump_CLKEN |= Clock_Pump_CLKEN_MASK;
	Clock_Pump_CLKSTBY |= Clock_Pump_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Pump_Stop
********************************************************************************
*
* Summary:
...
void Clock_Pump_Stop(void) 
{
    ...
    Clock_Pump_CLKEN &= (uint8)(~Clock_Pump_CLKEN_MASK);
	Clock_Pump_CLKSTBY &= (uint8)(~Clock_Pump_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Pump_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Pump_CLKEN & Clock_Pump_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Pump_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Pump_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Pump__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Pump_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Pump_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Pump_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Pump_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Pump_GetSourceRegister();
    uint16 oldDivider = Clock_Pump_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Pump_CLKEN & Clock_Pump_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Pump_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Pump_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Pump_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Pump_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Pump_MOD_SRC |= modeBitMask & (uint8)Clock_Pump_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Pump_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Pump_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Pump_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Pump_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Pump_MOD_SRC & (uint8)(Clock_Pump_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Pump_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Pump_GetDividerRegister();
    uint8 oldSrc = Clock_Pump_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Pump_MOD_SRC |= CYCLK_SSS;
        Clock_Pump_MOD_SRC =
            (Clock_Pump_MOD_SRC & (uint8)(~Clock_Pump_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Pump_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Pump_MOD_SRC & Clock_Pump_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Pump_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Pump_PHASE = clkPhase & Clock_Pump_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Pump_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Pump_PHASE & Clock_Pump_PHASE_MASK;
}
