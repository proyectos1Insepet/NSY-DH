...
{
    I2C_Bus_DISABLE, /* enableState */

    #if(I2C_Bus_FF_IMPLEMENTED)
        I2C_Bus_DEFAULT_XCFG,  /* xcfg */
        I2C_Bus_DEFAULT_CFG,   /* cfg  */

        #if(I2C_Bus_MODE_SLAVE_ENABLED)
            I2C_Bus_DEFAULT_ADDR, /* addr */
        #endif /* (I2C_Bus_MODE_SLAVE_ENABLED) */

        #if(CY_PSOC5A)
            LO8(I2C_Bus_DEFAULT_DIVIDE_FACTOR),  /* div */
        #else
            LO8(I2C_Bus_DEFAULT_DIVIDE_FACTOR), /* div1 */
            HI8(I2C_Bus_DEFAULT_DIVIDE_FACTOR), /* div2 */
        #endif /* (CY_PSOC5A) */

    #else  /* (I2C_Bus_UDB_IMPLEMENTED) */
        I2C_Bus_DEFAULT_CFG,    /* control */

        #if(CY_UDB_V0)
            I2C_Bus_INT_ENABLE_MASK, /* aux_ctl */

            #if(I2C_Bus_MODE_SLAVE_ENABLED)
                I2C_Bus_DEFAULT_ADDR, /* addr_d0 */
            #endif /* (I2C_Bus_MODE_SLAVE_ENABLED) */
        #endif /* (CY_UDB_V0) */
        ...
        I2C_Bus_DEFAULT_TMOUT_INTR_MASK,
            ...
            I2C_Bus_DEFAULT_TMOUT_PRESCALER_PRD,
        #endif /* (I2C_Bus_TIMEOUT_PRESCALER_ENABLED) */
...
*
* Global Variables:
*  I2C_Bus_backup - used to save component configuration and
...
void I2C_Bus_SaveConfig(void) 
{
        ...
        #if(CY_PSOC5A)
            I2C_Bus_backup.clkDiv   = I2C_Bus_CLKDIV_REG;
        #else
        ...
        I2C_Bus_backup.control = (I2C_Bus_CFG_REG & I2C_Bus_CTRL_ANY_ADDRESS_MASK);
            ...
            I2C_Bus_backup.intMask = I2C_Bus_INT_MASK_REG;

            #if(I2C_Bus_MODE & I2C_Bus_MODE_SLAVE)
                I2C_Bus_backup.addr = I2C_Bus_ADDR_REG;
            #endif /* (I2C_Bus_MODE & I2C_Bus_MODE_SLAVE) */
    ...
    #if(I2C_Bus_TIMEOUT_ENABLED)
        I2C_Bus_TimeoutSaveConfig();   /* Save Timeout config */
    #endif /* (I2C_Bus_TIMEOUT_ENABLED) */
}
...
void I2C_Bus_RestoreConfig(void) 
{
        ...
        if(I2C_Bus_CHECK_PWRSYS_I2C_BACKUP)    /* Enabled if was in Sleep */
        {
        ...
        else /* The I2C_REG_BACKUP was cleaned by PM API: it means Hibernate or wake-up not set */
        {
            ...
            #if(CY_PSOC5A)
                I2C_Bus_CLKDIV_REG  = I2C_Bus_backup.clkDiv;
            #else
            ...
            {
                I2C_Bus_SetPendingInt();   /* Generate interrupt to process incomming transcation */
            }
            I2C_Bus_EnableInt();
        #endif /* (I2C_Bus_WAKEUP_ENABLED) */
            ...
            I2C_Bus_INT_MASK_REG |= I2C_Bus_backup.intMask;

            enableInterrupts = CyEnterCriticalSection();
            I2C_Bus_INT_ENABLE_REG |= I2C_Bus_INT_ENABLE_MASK;
            CyExitCriticalSection(enableInterrupts);
...
{
    I2C_Bus_RestoreConfig();   /* Restore I2C register settings */
