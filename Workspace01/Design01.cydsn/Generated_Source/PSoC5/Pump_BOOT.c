         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  Pump_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(Pump_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(Pump_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional Pump_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != Pump_PACKET_EOP)
            {
                CyDelay(Pump_WAIT_EOP_DELAY);
            }
        }
